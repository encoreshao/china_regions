  ...
  module Helpers
    module FormHelper
      def region_select(object_name, fields, options = {}, html_options = {})
        ...
        dropdown_prefix = options[:prefix] ? "#{options[:prefix]}_" : ''

        output = if fields.is_a?(Array)
          render_fields(object_name, fields, options, html_options, dropdown_prefix, preselected_choices)
        else
          [render_field(object_name, fields, options, html_options, preselected_choices)]
        end
      ...
      def render_fields(object_name, fields, options, html_options, dropdown_prefix, preselected_choices)
        fields.each_with_index.map do |field, field_index|
          region_klass = field.to_s.classify.safe_constantize
          raise ArgumentError, "Method '#{field}' is not a valid attribute of #{object_name}" unless region_klass
          ...
          next_field = fields.at(field_index + 1)

          set_prompt(field, options, region_klass)
          set_html_options(object_name, field, html_options, next_field, dropdown_prefix)

          options[:selected] = field == :province ? preselected_choices[:province_id] : nil

          select(object_name, "#{dropdown_prefix}#{field}_id", choices, options, html_options)
        end
      end

      def render_field(object_name, field, options, html_options, preselected_choices)
        attribute, field = attribute_and_field_for(field)

        region_klass = field.to_s.classify.safe_constantize
        raise ArgumentError, "Method '#{field}' is not a valid attribute of #{object_name}" unless region_klass

        options[:prompt] = region_prompt(region_klass)
        options[:selected] = preselected_choices[:province_id] if field == :province && preselected_choices[:province_id]

        select(object_name, attribute, region_options(region_klass), options, html_options)
      end

      def attribute_and_field_for(field)
        return [field, field.to_s.delete_suffix('_id').to_sym] if field.to_s.end_with?('_id')

        ["#{field}_id".to_sym, field]
      end
      ...
      def region_options(region_klass)
        @region_options ||= {}
        @region_options[region_klass] ||= region_klass.where(nil).collect { |p| [p.name, p.id] }
      end
      ...
      def set_html_options(object_name, method, html_options, next_region, prefix)
        html_options[:data] ||= {}
        html_options[:data][:region_klass] = method.to_s

        if next_region
          normalized_object_name = object_name.to_s.tr('[', '_').delete(']')
          html_options[:data].merge!(
            region_target: "#{normalized_object_name}_#{prefix}#{next_region}_id",
            region_target_klass: next_region.to_s
          )
        else
    ...
    module FormBuilder
      def region_select(methods, options = {}, html_options = {})
        @template.region_select(@object_name.to_s, methods, options, html_options)
      end
    end
  end
end
...
ActionView::Helpers::FormBuilder.include ChinaRegions::Helpers::FormBuilder
